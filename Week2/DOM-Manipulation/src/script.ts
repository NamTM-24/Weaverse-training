...
let todos: Todo[] = [];
const todoInput = document.querySelector<HTMLInputElement>('#newTodo');
const todoList = document.querySelector<HTMLUListElement>('#todoList');
const todoCount = document.querySelector<HTMLParagraphElement>('#todoCount');
const buttonAdd = document.querySelector<HTMLButtonElement>('#addBtn');
...
function createLI_and_Delete(todo: Todo): void {
    if (!todoList) return;
...
function updateTodoUnfinished(): void {
    if (!todoCount) return;
